...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:header class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:dropdown position="top" align="end">
            ...
            <flux:menu>
                ...
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
                    <flux:menu.item as="button" type="submit" icon="log-out" class="w-full" variant="danger">
                        {{ __('Log Out') }}
                    </flux:menu.item>
                </form>
            </flux:menu>
        ...
        @else
        <flux:button :href="route('login')" wire:navigate size="sm" variant="primary" icon="log-in">
            Log in
        </flux:button>
        @endauth
    </flux:header>
